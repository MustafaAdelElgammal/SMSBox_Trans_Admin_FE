...
(function($) {
    ...
    //
		...
		defaultCloseEffect: function(target, closeditems, targetclass, bodyclass, funcs) {
			...
			$(target).removeClass(targetclass + ' toggled');
			$('body').removeClass(bodyclass);
      const items = closeditems.filter (function (value, index, array) {
        return array.indexOf (value) === index;
      });
      if (items.length > 0) {
				setTimeout(function() {
          items.forEach(function(item) {
            const s = +$(item.toString()).find('li > a.active').length > 0;
            if(s){
                $(item.toString()).parent().addClass('u-side-nav-opened');
                $(item.toString()).slideDown(400);
                funcs();
            }
          })
				}, 300);
